...
REM Ajouter ceci avant de créer le WAR pour être sûr que les JSP sont inclus
echo "Vérification des fichiers JSP..."
if not exist "%BUILD_DIR%\accueil.jsp" (
    echo "ATTENTION: accueil.jsp manquant!"
    pause
)
if not exist "%BUILD_DIR%\listeDepenses.jsp" (
    echo "ATTENTION: listeDepenses.jsp manquant!"
    pause
)
if not exist "%BUILD_DIR%\formDepense.jsp" (
    echo "ATTENTION: formDepense.jsp manquant!"
    pause
)
